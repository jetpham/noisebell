...
impl GpioMonitor {
        ...
        F: FnMut(CircuitEvent) + Send + 'static,
    {
        self.pin.set_async_interrupt(
            ...
            move |event| {
                let circuit_event = match event.trigger {
                    Trigger::RisingEdge => CircuitEvent::Closed,
                    Trigger::FallingEdge => CircuitEvent::Open,
                    _ => return, // Should never happen with Trigger::Both
                };
                callback(circuit_event);
            },
        ).context("Failed to set async interrupt")
    }

    #[allow(dead_code)] // This will be used later for the rest api
